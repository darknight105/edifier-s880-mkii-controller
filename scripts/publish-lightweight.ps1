...
$outputDirectory = Get-FullPath (Join-Path $artifactsRoot 'lightweight')
$workRoot = Get-FullPath (Join-Path $artifactsRoot '.publish-work\lightweight')
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Framework-dependent CLI publish failed with exit code $LASTEXITCODE" }

    $backend = Join-Path $payloadDirectory 's880ctl.exe'
    if (-not (Test-Path -LiteralPath $backend)) { throw 'Framework-dependent CLI publish did not produce s880ctl.exe.' }
    ...
    & dotnet publish $guiProject @commonPublishArgs `
        ...
        '-o' $publishDirectory
    if ($LASTEXITCODE -ne 0) { throw "Framework-dependent GUI publish failed with exit code $LASTEXITCODE" }

    $publishedSourceExecutable = Join-Path $publishDirectory 'S880Controller.exe'
    if (-not (Test-Path -LiteralPath $publishedSourceExecutable)) { throw 'Lightweight GUI publish did not produce S880Controller.exe.' }
    ...
    $report = [ordered]@{
        ...
        separateDesktopRuntimeRequired = 'Microsoft.NETCore.App 10.x and Microsoft.WindowsDesktop.App 10.x x64'
